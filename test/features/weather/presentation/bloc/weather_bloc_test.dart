...
void main() {
  ...
  blocTest<WeatherBloc, WeatherState>(
    'emits [WeatherLoading, WeatherSuccess] when OnGeoLocationChanged is added.',
    build: () {
      when(mockGetCurrentWeatherUseCase.execute(lat, lng))
          .thenAnswer((_) async => const Right(weatherEntity));
      return weatherBloc;
    },
    act: (bloc) => bloc.add(const OnGeoLocationChanged(lat, lng)),
  ...
  blocTest<WeatherBloc, WeatherState>(
    ...
    build: () {
      when(mockGetCurrentWeatherUseCase.execute(lat, lng)).thenAnswer(
