...
void main() {
  ...
  group("get current wheather", () {
    test("should get weather model on successfull call", () async {
      when(mockWeatherRemoteDataSource.getCurrentWeather(lat, lng)).thenAnswer(
        (_) async => weatherModel,
      );
      final result = await weatherRepositoryImpl.getCurrentWeather(lat, lng);

      expect(result, equals(const Right(weatherEntity)));
    });
    test("should get an error on unsuccessfull call", () async {
      when(mockWeatherRemoteDataSource.getCurrentWeather(lat, lng))
    ...
    test("should get an error if device is not connected to internet",
        () async {
      when(mockWeatherRemoteDataSource.getCurrentWeather(lat, lng)).thenThrow(
